...
export function Footer({ content, lang }: { content: Content; lang: Lang }) {
  ...
  return (
    <footer className="on-dark relative bg-ink pb-28 pt-16 text-paper sm:pb-12">
      <div className="container-x">
        <div className="grid gap-12 lg:grid-cols-[1.3fr_1fr_1fr]">
          <div>
            <span className="inline-flex rounded-2xl bg-paper px-4 py-2.5">
              <Logo src={site.logoSrc} name={site.brand[lang]} />
            </span>
            <p className="mt-6 max-w-sm text-[0.95rem] leading-relaxed text-paper/70">{footer.about}</p>
          ...
          <nav aria-label={ui.sectionsNav}>
            <ul className="space-y-3">
              {links.map((l) => (
                <li key={l.id}>
                  <a href={`#${l.id}`} className="text-[0.95rem] text-paper/75 transition hover:text-gold-soft">
                    {l.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-paper/45">{footer.contacts}</p>
            <ul className="mt-4 space-y-3 text-[0.95rem] text-paper/75">
            ...
            <p className="mt-7 text-xs font-semibold uppercase tracking-[0.18em] text-paper/45">{footer.social}</p>
              ...
              {site.socials.map((s) => (
                <li key={s.id}>
                  <a
                    ...
                    className="inline-flex rounded-full border border-paper/20 px-3.5 py-1.5 text-sm text-paper/80 transition hover:border-gold-soft hover:text-gold-soft"
                  >
                    {s.label}
        ...
        <div className="mt-14 flex flex-wrap items-center justify-between gap-4 border-t border-paper/10 pt-6 text-sm text-paper/50">
          <p>
